...
function importSettings(filename, defaultValues = defaultSettings) {
    ...
    const settings = { ...defaultValues, id, ...readJsonFile(filename) };
...
function resolveFilename(filename, relativeTo) {
    ...
    try {
        return resolve_from_1.default(relativeTo, filename);
    }
    catch (error) {
        ...
        return resolve_from_1.default(relativeTo, `./${normalizedFilename}`);
    }
}
...
function checkFilenameMatchesGlob(filename, globs) {
    ...
    const matches = globs
        .filter(g => minimatch_1.default(filename, g, { matchBase: true }));
