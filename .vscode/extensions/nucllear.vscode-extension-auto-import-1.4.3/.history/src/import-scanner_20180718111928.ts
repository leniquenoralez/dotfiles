...
export class ImportScanner {
    ...
    public scan(request : any) : void {
        ...
        this.showOutput = request.showOutput
            ? request.showOutput
            : false;
    ...
    private processWorkspaceFiles(files : vscode.Uri[]) : void {
        let pruned = files.filter((f) => {
            return f
                ...
                .indexOf('typings') === -1 && f
                .fsPath
                .indexOf('node_modules') === -1 && f
                .fsPath
                .indexOf('.history') === -1 && f
                .fsPath
                .indexOf('jspm_packages') === -1;
        });
    ...
    private loadFile(file : vscode.Uri, last : boolean) : void {
        FS.readFile(file.fsPath, 'utf8', (err, data) => {
            ...
            if (last && this.showOutput && this.showNotifications) {
                ...
                let str = `[AutoImport] cache creation complete - (${Math.abs(
                    <any>this.scanStarted -
                        <any>this.scanEnded)}ms)`; vscode.window .showInformationMessage(str); } }); }
                            private processFile(data: any, file: vscode.Uri): void {// added code to support any other middleware that the component can  be nested
                            // in.
